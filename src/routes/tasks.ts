...
tasks.post('/new-post-handler', async (c) => {
  ...
  const lock_key = 'new-post-handler-lock'; // TODO: Add timer remove lock after 35 seconds just in case something goes wrong and the lock doesn't get released
  ...
  return c.json<TaskResponse>({ status: result['status'], message: result['message']}, result['number']);
});