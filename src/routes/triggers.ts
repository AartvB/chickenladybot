...
triggers.post('/on-app-upgrade', async (c) => {
  ...
  await redis.del('posts'); // TODO: REMOVE THIS LINE! But for testing, empty the set of all posts
  await redis.del('posts-of-Aartvb') // TODO: Remove this line as well, it's only for testing to reset the count of posts by the bot
  await redis.del('new_post_queue'); // Clear the queue of new posts to be processed
  ...
  // TODO: set current-count to the correct count based on the existing posts in the subreddit, in case the bot was offline for a while and missed some posts.
